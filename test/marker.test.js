...
test('convertPdfToMarkdown can materialize markdown via the local paddleocr-vl wrapper', async () => {
  ...
  try {
    ...
    await fs.writeFile(
      ...
      [
        ...
        'printf "# PaddleOCR-VL Parsed\\n\\n## Abstract\\n\\nGenerated by fake PaddleOCR-VL.\\n" > "$output"'
      ].join('\n'),
      { mode: 0o755 }
    );
    ...
    assert.match(markdown, /Generated by fake PaddleOCR-VL/);
  } finally {
    await fs.rm(tempDir, { recursive: true, force: true });
  }
});
...
test('convertPdfToMarkdown fails fast when paddleocr-vl setup or inference fails', async () => {
  ...
  try {
    ...
    await assert.rejects(
      ...
      /PaddleOCR-VL failed|paddleocr install_hpi_deps gpu/i
    );
  } finally {
    await fs.rm(tempDir, { recursive: true, force: true });
  }
