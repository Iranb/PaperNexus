...
- `literature_discovery ingest`, `import_and_process`, or `processImports=true` asks PaperNexus to process imports inline, defaulting to progressive worker-side logical batching with `importBatchEnabled=true`, `importBatchInitialTasks=4`, `importBatchMaxTasks=16`, and `importBatchCoalesceMs=0`; this can still be long-running because parsing, semantic extraction, fast commit, and authoritative graph sync can lag behind discovery.
- `import_workflow submit` only accepts tasks into the queue; the MCP serve import worker defaults to `imports.batchEnabled=true`, `batchProgressive=true`, `batchInitialTasks=4`, `batchMaxTasks=16`, and `batchCoalesceMs=0`, so multiple pending tasks can share one graph commit unless server config explicitly disables batching.
...
When graph build is delayed, use `literature_discovery_progress` for current discovery state, `literature_discovery status` / `report` for the paper list, and `import_workflow queue_progress`, `status`, or `wait` for graph-readiness. If an MCP client has a short request timeout, call the same import read with `async=true` or use `operation=submit_async` plus `asyncOperation`, then poll `operation=async_status` with the returned `jobId`. `import_workflow wait` now waits for the authoritative sync job by default; pass `waitForAuthoritativeSync=false` only when you intentionally want raw import-task completion. Do not rerun graph queries just because discovery finished.
