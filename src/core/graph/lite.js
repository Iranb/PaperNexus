...
export function getLiteNodeSearchTokens(node) {
  const text = [
    ...
    Array.isArray(node.properties?.identifierKeys) ? node.properties.identifierKeys.join(' ') : '',
    node.properties?.doi,
    node.properties?.arxivId,
    node.properties?.pmid,
    node.properties?.pmcid,
    node.properties?.isbn,
    node.properties?.issn,
    node.properties?.canonicalId,
    node.properties?.normalizedTitle,
    Array.isArray(node.properties?.identityAliases) ? node.properties.identityAliases.join(' ') : '',
    node.properties?.sourceId,
