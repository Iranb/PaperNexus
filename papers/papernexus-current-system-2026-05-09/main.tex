...
\section{Experiment Run Records}
...
\subsection{Execution Environments and Artifact Roots}

\Cref{tab:appendix-run-environments} records the two environments used for the local pipeline and public benchmark measurements. The local workstation was used for authenticated upload, queue, and qwen/CPA LLM checks. The 41 server was used for sub-5GB public benchmark runs and the scratch Kuzu import.

\begin{table}[t]
\centering
\caption{Execution environments used by the reported audits.}
\label{tab:appendix-run-environments}
\footnotesize
\begin{tabularx}{\linewidth}{@{}l Y Y@{}}
\toprule
Environment & Recorded machine context & Workload role \\
\midrule
Local workstation & macOS 15.1 on \code{arm64}; 10 logical CPUs; 16GB RAM; Node \code{v25.4.0}. & Authenticated PDF upload, import queue recovery, heuristic-only import stress, and qwen/CPA LLM batching. \\
41 server & Host \code{jin-Super-Server}; Linux 6.8.0-90-generic; AMD EPYC 7542 32-Core Processor; 64 logical CPUs; 125GiB RAM; Python 3.10.16; Node \code{v22.16.0}. & BEIR, LitSearch, ScholarGym, live-provider diagnostics, and STaRK-MAG/Kuzu scratch runs. \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{lstlisting}[style=pncode]
/data2/hyq/papernexus-runtime/node-current/bin/node ./src/cli/index.js serve --host 0.0.0.0 --port 4821
\end{lstlisting}

\subsection{Local Pipeline and LLM Audit Records}

\Cref{tab:appendix-local-run-records} expands the local upload, queue, and pipeline measurements from the main text. \Cref{tab:appendix-qwen-batch-records} separately records the qwen batch checks, which used the local OpenAI-compatible \code{cliproxyapi} configuration with thinking disabled.

\begin{table}[t]
\centering
\caption{Detailed local audit records for upload, import, queue recovery, and LLM-enhanced pipeline timing.}
\label{tab:appendix-local-run-records}
\footnotesize
\begin{tabularx}{\linewidth}{@{}l Y Y@{}}
\toprule
...
\begin{table}[t]
\centering
\caption{Detailed qwen/CPA batch extraction records. Each condition was executed once.}
\label{tab:appendix-qwen-batch-records}
\footnotesize
...
\subsection{Sub-5GB Fixed-Corpus Benchmark Records}

\Cref{tab:appendix-fixed-corpus-runtime-records} records the fixed-corpus runtime artifacts executed on the 41 server, and \Cref{tab:appendix-fixed-corpus-metric-records} records the full-precision metrics. All runs used \code{--evaluation-mode fixed-corpus}, \code{--max-candidates 1000}, \code{--k 1,5,10,20,50,100}, and \code{PAPERNEXUS_FIXED_CORPUS_SCAN_LIMIT=5000}. ScholarGym additionally required \code{NODE_OPTIONS=--max-old-space-size=16384}.

\begin{table}[t]
\centering
\caption{Detailed fixed-corpus runtime records from the 41 server.}
\label{tab:appendix-fixed-corpus-runtime-records}
\footnotesize
\begin{tabularx}{\linewidth}{@{}l l l Y@{}}
\toprule
Dataset & Scale & Wall / peak RSS & Artifact and caveat \\
\midrule
BEIR TREC-COVID & 50 queries; 171,332 papers. & 8:54.15 / 2.56GB. & Artifact directory \code{trec-covid-fixed-scan5k}. Large-qrels biomedical regression trace, not a repeated BEIR leaderboard run. \\
BEIR SciDocs & 1,000 queries; 25,657 papers. & 6:38.92 / 1.19GB. & Artifact directory \code{scidocs-fixed-scan5k}. Citation-oriented paper-retrieval regression trace. \\
LitSearch full & 597 queries; 64,183 papers. & 2:44.21 / 1.47GB. & Artifact directory \code{litsearch-full-fixed-scan5k}. Full 597-query run, but not yet the official LitSearch broad/specific metric slices. \\
...
\begin{table}[t]
\centering
\caption{Full-precision fixed-corpus metric records. Pool is candidate-pool recall and Zero is zero-match rate.}
\label{tab:appendix-fixed-corpus-metric-records}
\scriptsize
\begin{tabular}{@{}l r r r r r r r r@{}}
\toprule
Dataset & nDCG10 & R10 & MRR10 & Hit100 & R100 & nDCG100 & Pool & Zero \\
...
\subsection{Live Provider Diagnostic Records}

\Cref{tab:appendix-live-diagnostic-records} records the live-provider checks. These runs were intentionally treated as diagnostics because provider throttling, query shape, missing gold identifiers, and date-stamped API behavior are part of what they measure.

\begin{table}[t]
\centering
\caption{Detailed live-provider diagnostic records from the 41 server.}
\label{tab:appendix-live-diagnostic-records}
\scriptsize
\begin{tabularx}{\linewidth}{@{}l Y Y@{}}
\toprule
Diagnostic & Runtime record & Observed failure mode \\
\midrule
AutoResearchBench sample & 40 queries; 175 relevant papers; artifact directory \code{autoresearchbench-live40}; wall 5:24.77; average query latency 8,110.7ms; peak RSS 100,232KB. & Hit@10, R@10, and all retrieval metrics were 0; zero-match rate was 1.000. Provider failures included 58 OpenAlex HTTP 400 responses and 5 Semantic Scholar HTTP 429 responses. The run shows that clue-style tasks need query rewriting/decomposition before retrieval quality can be judged. \\
ScholarSearch diagnostic & Intended max-10 live run, but the CLI evaluated 50 queries; artifact directory \code{scholarsearch-live10}; wall 4:14.39; peak RSS 90,300KB. & No usable gold-paper set was available, so ranking metrics are not meaningful. Average retrieved count was 0.26, and the run recorded one Semantic Scholar HTTP 429 response. The run remains a live trace diagnostic only. \\
\bottomrule
\end{tabularx}
\end{table}
...
\subsection{STaRK-MAG and Kuzu Diagnostic Records}

\Cref{tab:appendix-stark-import-records} records the STaRK-MAG scratch import, and \Cref{tab:appendix-stark-retrieval-records} records the retrieval diagnostic. The dataset copy lived under \code{/data1/hyq/papernexus-sub5g-benchmarks/datasets/stark-mag}. Its raw plus extracted footprint was approximately 3.5GB. The extracted graph contained 1,872,968 nodes and 19,919,698 edges.

\begin{table}[t]
\centering
\caption{Detailed STaRK-MAG/Kuzu scratch import records.}
\label{tab:appendix-stark-import-records}
\footnotesize
\begin{tabularx}{\linewidth}{@{}l l Y@{}}
\toprule
Stage & Wall / peak RSS & Observed record \\
\midrule
CSV preparation & 1:02.87 / 6.91GB. & Wrote import files for 1,872,968 nodes and 19,919,698 edges plus seed lists for 84 human-eval queries. The manifest was saved as \code{prep-manifest.json}. \\
Kuzu import & 0:10.24 / 2.35GB. & Imported all nodes and edges into a scratch Kuzu database. The scratch artifact footprint was 1.4GB. Pipe-delimited import files were used after comma/quote CSV issues appeared. Summary saved as \code{kuzu-import-summary.json}. \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{table}[t]
\centering
\caption{Detailed STaRK-MAG/Kuzu retrieval metric records over 84 human-eval queries.}
\label{tab:appendix-stark-retrieval-records}
\footnotesize
...
The STaRK-MAG diagnostic took 400,994ms in total, with average latency 4,773.68ms/query, p50 5,018ms, p95 6,865ms, max 8,135ms, and peak RSS 5.06GB. The report was saved as \code{stark-mag-kuzu-diagnostic-report.json}. The lexical-only row is a seed baseline, and the graph-expanded row is a one-to-two-hop diagnostic rather than the official STaRK protocol. The graph step improved top-100 candidate coverage but did not improve Hit@10, so ranking still needs relation-aware scoring and high-degree penalties.
...
\subsection{Detailed-Record Caveats}
