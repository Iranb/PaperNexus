...
\paragraph{Local pipeline measurements.}
On May 11, 2026, we ran several local measurements to separate the cost of graph commit, synchronization, PDF parsing, and LLM-based semantic enrichment. The tests were run against local \papernexus{} corpora and authenticated local server surfaces. They are engineering measurements rather than a public benchmark suite, but they are useful for locating the current bottleneck and for validating the migration plan toward larger corpora.

\begin{table}[t]
\centering
\caption{Stage timing from a 10-real-paper local pipeline run. Values are means reported by the pipeline progress logs for that run.}
...
\begin{table}[t]
\centering
\caption{Production-style import timing shape after scoped import processing was enabled. These values describe normal single-paper import tasks and are separate from the 10-paper aggregate pipeline log above.}
...
\begin{table}[t]
\centering
\caption{Queue recovery and continue validation from a production-style import incident.}
...
\begin{table}[t]
\centering
\caption{Single-prompt multi-paper LLM extraction test using local \code{cliproxyapi} with \code{qwen3.6-plus} and thinking disabled.}
...
\begin{table}[t]
\centering
\caption{Authenticated PDF upload and import stress measurements. Each run used a fresh temporary corpus with one seed Markdown source, so committed paper counts are one larger than the number of uploaded PDFs.}
