...
def build_headers(token: str, with_json: bool = False) -> typing.Dict[str, str]:
    headers = {
...
def request_json(method: str, api_base: str, path: str, token: str, payload: typing.Optional[dict] = None, timeout: float = DEFAULT_TIMEOUT):
    body = None
...
def build_remote_paths(local_root: Path, files: list[Path], remote_dir: str) -> typing.List[str]:
...
def build_default_remote_dir(local_root: Path, remote_root: typing.Optional[str] = None) -> str:
...
def find_task_record(registry: dict, paper_id: str = "", source: str = "", task_id: str = "", corpus: str = "") -> tuple[typing.Optional[dict], typing.Optional[str]]:
    matches = []
...
def has_registry_value(value) -> bool:
...
def upsert_task_record(record: dict) -> Path:
    ...
    if existing is not None:
        existing.update({key: value for key, value in record.items() if has_registry_value(value)})
    else:
        registry["tasks"].append(record)
    return save_task_registry(registry)


def build_registry_summary(record: typing.Optional[dict] = None, matched_by: typing.Optional[str] = None) -> dict:
    summary = {
...
def build_graph_payload(corpus: str, query: str, options: typing.Optional[dict] = None) -> dict:
    payload = {
